...
treatments_df <- as.data.frame(colnames(df_pfam))
treatments_df <- treatments_df %>% 
  mutate(Treatment = case_when(
    grepl("712", `colnames(df_pfam)`, fixed = TRUE) ~ "712",
    grepl("722", `colnames(df_pfam)`, fixed = TRUE) ~ "722",
    grepl("CCD8", `colnames(df_pfam)`, fixed = TRUE) ~ "CCD8",
    grepl("MMA", `colnames(df_pfam)`, fixed = TRUE) ~ "MMA",
    grepl("GUS", `colnames(df_pfam)`, fixed = TRUE) ~ "GUS",
    grepl("EP", `colnames(df_pfam)`, fixed = TRUE) ~ "EP",
  )) %>%
...
#ARGUMENTS
#x = dataframe 
#treat_group_factor = treatment groups from info df, as df_info_pfam$Treatment
...
df_few0_pfam <- df_pfam[filter_fun(df_pfam,df_info_pfam$Treatment, 0.25, 0.75),]
df_few0_pfam <- df_few0_pfam[rowSums(df_few0_pfam)>0,]
ko_T <- ko_T[rowSums(ko_T)>0,]
df_fewer0_pfam <- df_pfam[filter_fun(df_pfam,df_info_pfam$Treatment, 0.18, 0.85),]
unique(df_info_pfam$Treatment)
```
...
#712-GUS
df_712_GUS <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,5)]]
df_few0_712_GUS <- df_712_GUS[filter_fun(df_712_GUS,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,5)]],0.18, 0.85),]

#712-MMA
#df_712_MMA <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,6)]]
#df_few0_712_MMA <- df_712_MMA[filter_fun(df_712_MMA,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,6)]],0.25, 0.75),]

#712_EP
#df_712_EP <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,4)]]
#df_few0_712_EP <- df_712_EP[filter_fun(df_712_EP,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,4)]],0.25, 0.75),]

#722-GUS
df_722_GUS <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(2,5)]]
df_few0_722_GUS <- df_722_GUS[filter_fun(df_722_GUS,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(2,5)]],0.18, 0.85),]

#722-MMA
#df_722_MMA <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(2,6)]]
#df_few0_722_MMA <- df_722_MMA[filter_fun(df_722_MMA,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(2,6)]],0.18, 0.85),]

#722_EP
#df_722_EP <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(2,4)]]
#df_few0_722_EP <- df_722_EP[filter_fun(df_722_EP,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(2,4)]],0.18, 0.85),]

#CCD8-GUS
df_CCD8_GUS <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(3,5)]]
df_few0_CCD8_GUS <- df_CCD8_GUS[filter_fun(df_CCD8_GUS,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(3,5)]],0.18, 0.85),]

#CCD8-MMA
#df_CCD8_MMA <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(3,6)]]
#df_few0_CCD8_MMA <- df_CCD8_MMA[filter_fun(df_CCD8_MMA,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(3,6)]],0.18, 0.85),]

#CCD8_EP
#df_CCD8_EP <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(3,4)]]
#df_few0_CCD8_EP <- df_CCD8_EP[filter_fun(df_CCD8_EP,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(3,4)]],0.18, 0.85),]

#2 Controls 
#GUS-MMA 
df_GUS_MMA <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(5,6)]]
df_few0_GUS_MMA <- df_GUS_MMA[filter_fun(df_GUS_MMA,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(5,6)]],0.18, 0.85),]


#all but EP
df_all_plant <- df_pfam[,df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,2,3,5,6)]]
df_few0_plant <- df_all_plant[filter_fun(df_all_plant,df_info_pfam$Treatment[df_info_pfam$Treatment %in% unique(df_info_pfam$Treatment)[c(1,2,3,5,6)]],0.18, 0.85),]
df_info_pfam_plant <- df_info_pfam[df_info_pfam$Treatment!= 'EP',]
...
#Sum normalization 
df_few0_pfam_sum <- decostand(df_few0_pfam,method = "total",2)

#Hellinger transform
df_few0_pfam_hellinger <- decostand(df_few0_pfam,method = "hellinger",2)
...
df_few0_pfam_clr <- as.matrix(as.data.frame(t(clr(min(df_few0_pfam[df_few0_pfam>0])/10+t(df_few0_pfam)))))

#presudocount: replacing zero values by 1 
df_few0_pfam_psc <- df_few0_pfam
df_few0_pfam_psc[df_few0_pfam_psc==0] <- 1
#then apply CLR to the dataframe 
gems_few0groups2_clr2 <- as.matrix(as.data.frame(t(clr(t(df_few0_pfam_psc)))))
...
```{r}
###PCA with clr data
few0_funct_pca_clr <- prcomp(t(df_few0_pfam_clr))
plot(few0_funct_pca_clr$x,
     col=rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))],
...
autoplot(few0_funct_pca_clr, data=t(df_few0_pfam_clr), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))]) 


#### PCA with log transformed 0 to 1 pseudocount data
few0_funct_pca_log <- prcomp(t(log(df_few0_pfam_psc)))
...
autoplot(few0_funct_pca_log, data=t(log(df_few0_pfam_psc)), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])


### PCA with hellinger transformation
few0_funct_pca_hellinger <- prcomp(t(df_few0_pfam_hellinger))
plot(few0_funct_pca_hellinger$x,
     col=rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))],
     main='PCA of Hellinger transformed data', pch=19)
legend("topright",col=rainbow(6),
       legend=levels(as.factor(df_info_pfam$Treatment)),
       lty=1,
       bty="n")

autoplot(few0_funct_pca_hellinger, data=t(df_few0_pfam_hellinger), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])


### PCA with sum normalization 
few0_funct_pca_sum <- prcomp(t(df_few0_pfam_sum))
...
autoplot(few0_funct_pca_sum, data=t(df_few0_pfam_sum), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])
```

### PCA for plant groups 
```{r}
#plant treatment groups
###PCA with clr data
few0_plant_pca_clr <- prcomp(t(df_few0_plant_clr))
plot(few0_plant_pca_clr$x,
     col=rainbow(6)[as.numeric(as.factor(df_info_pfam_plant$Treatment))],
     main='PCA of CLR transformed data')
legend("topright",col=rainbow(6),
       legend=levels(as.factor(df_info_pfam_plant$Treatment)),
       lty=1,
       bty="n")

autoplot(few0_plant_pca_clr, data=t(df_few0_plant_clr), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam_plant$Treatment))]) 
...
autoplot(few0_plant_pca_log, data=t(df_few0_plant_psc), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam_plant$Treatment))])
...
autoplot(few0_plant_pca_hellinger, data=t(df_few0_plant_hellinger), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam_plant$Treatment))])
...
autoplot(few0_plant_pca_sum, data=t(df_few0_plant_sum), colour = rainbow(6)[as.numeric(as.factor(df_info_pfam_plant$Treatment))], label = TRUE)
```
...
#library size
#logistic
prepro <- feature_table_pre_process(feature_table = df_few0_pfam[rowSums(df_few0_pfam[,df_info_pfam$Treatment %in% c("GUS","712")])>0,df_info_pfam$Treatment %in% c("GUS","712")], 
                                   meta_data = df_info_pfam[df_info_pfam$Treatment %in% c("GUS","712"),], 
                                   sample_var = "Sample", 
                                   group_var = "Treatment", 
                                   out_cut = 0.05, 
                                   zero_cut = 0.90, 
...
which(prepro$structure_zeros %in% df_few0_pfam)
print(prepro)
...
```{r}
differential <- ANCOM(feature_table = df_few0_pfam_sum,
                      meta_data = df_info_pfam,
                      main_var = 'Treatment',
                      p_adj_method = "BH",
                      alpha = 0.5,
                      adj_formula = NULL)
...
```{r}
permanova <- adonis(t(df_few0_pfam_clr) ~ Treatment,
               data = df_info_pfam, permutations=99, method = "euclidian")
...
anova(betadisper(euc1, df_info_pfam$Treatment))
...
df_info_pfam_712_GUS <- df_info_pfam[df_info_pfam$Treatment=='712' | df_info_pfam$Treatment=='GUS',]

permanova_712_GUS <- adonis(t(df_few0_712_GUS_clr) ~ Treatment,
               data = df_info_pfam_712_GUS, permutations=99, method = "euclidian")
...
col_counts <- as.data.frame(colSums(df_pfam))
colnames(col_counts) <- c("Total_mapped_Reads")
#Plot of total mapped counts per sample 
plot(col_counts$`Total_mapped_Reads`, ylab = 'Total Mapped Reads', xlab = 'Sample', xlim = c(0,55), ylim = c(25000000,40000000), col =rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])

plot(colSums(df_few0_pfam_clr), main= "CLR", col=rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])

plot(colSums(df_few0_pfam_sum), main ="SUM", col=rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])

plot(colSums(df_few0_pfam_hellinger), main="hellinger", col=rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])
```
...
hist(colSums(df_few0_pfam_clr),xlab="Total Counts per Sample",main="Histogram of CLR Mapped Counts Distribution",las=1)
hist(colSums(df_few0_pfam_sum),xlab="Total Counts per Sample",main="Histogram of SUM Mapped Counts Distribution",las=1)
hist(colSums(df_few0_pfam_hellinger),xlab="Total Counts per Sample",main="Histogram of Hellinger Mapped Counts Distribution",las=1)
...
boxplot(df_pfam, col = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])
#
boxplot(df_few0_pfam_clr, col = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])

boxplot(df_few0_pfam_sum, col = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])

boxplot(df_few0_pfam_hellinger, col = rainbow(6)[as.numeric(as.factor(df_info_pfam$Treatment))])

```

```{r}
#pseudo count replace 0 values for 1 
df_pfam_1 <- replace(df_pfam, df_pfam==0, 1)
#plot the distribution of all mapped counts per sample on a logarithmic scale
boxplot(log(df_pfam_1), col = c("royalblue1","tomato1","firebrick4","yellow","purple","green")[as.numeric(as.factor(treatments_df$Treatment))])
...
```{r}
rarecurve(as.data.frame(t(df_pfam)), step=1000000, label = FALSE, ylim= c(7000,10000), xlim = c(5000000, 40000000), cex=0.6,
#quickRareCurve(as.data.frame(t(df_pfam)), max.cores = F, nCores = 6)
          col = c("royalblue1","tomato1","firebrick4","yellow","purple","green")[as.numeric(as.factor(treatments_df$Treatment))])
abline(v=min(colSums(df_pfam)),lty=3)
...
row_index <- grep("EutC", rownames(df_pfam))
row_index
#get count value based on row index for a specific sample 
df_pfam$T_MMA_3[[row_index]]
...
length(which(df_pfam==0))/(nrow(df_pfam) * ncol(df_pfam))
```
...
richness_1_noNorm <- as.data.frame(apply(df_pfam,
                           2,
                           function(x) length(which(x>0))))

summary(richness_1_noNorm)
colnames(richness_1_noNorm) <- 'Richness'
richness_1_noNorm$Treatment <- df_info_pfam$Treatment
...
boxplot(richness_1_noNorm ~ df_info_pfam$Treatment,
...
#print the value 
for (i in 1:ncol(df_pfam))
{
  maxx <- which.max(df_pfam[,i])
  #print(df_pfam[[maxx]])
  print(names(df_pfam)[i])
  cat(colnames(df_pfam[,i]), df_pfam[maxx,i], rownames(df_pfam[maxx,]), "\n")
}
#ABC_tran
```
...
plot(df_pfam[,"T_722_9"], col = 'yellow')
points(df_pfam[,"T_GUS_18"], col = 'orange')
points(df_pfam[,"T_CCD8_10"], col = 'red')
points(df_pfam[,"T_712_2"], col='blue')
points(df_pfam[,"T_MMA_1"], col = 'purple')
points(df_pfam[,"T_EP_1"], col = 'green')
#clearly skewed as shown in the boxplot 

#pairwise plot between 2 samples
#plot(df_pfam[,1], df_pfam[,30], col = 'red')

#plot(df_pfam, col = c("royalblue1","tomato1","firebrick4","yellow","purple","green")[as.numeric(as.factor(treatments_df$Treatment))])
...
```{r}
#d <- density(df_pfam$T_MMA_12) 
#plot(d)
barplot(df_pfam$T_MMA_12, col = 'red')
...
zero_df <- as.data.frame(rowSums(df_pfam == 0))
...
#cluster <- agnes(df_pfam$T_MMA_12, diss = TRUE, method = "average")
#plot(cluster, which.plots = 2, hang = -1, label = sample, main = "", axes = FALSE, xlab = "", ylab = "", sub = "")
#heatmap(df_pfam$T_MMA_12, scale = "none")
```
...
ratespermillion <- as.data.frame(sweep(10^6*df_pfam,2,totalcounts,FUN="/"))
...
```{r}
#SUM NORMALIZATION 
sum_norm_counts <- as.data.frame(sweep(df_pfam,2,totalcounts,FUN="/"))
...
mappability <- as.matrix(df_pfam)
#matplot(1:11,mappability, type="l", lwd=1)
#df_pfam[is.na(df_pfam) | df_pfam=='Inf'] = NA
#df_pfam[rowSums(is.na(df_pfam))>0,]
df_pfam_1 <- replace(df_pfam, df_pfam==0, 1)

log_data_1 <- log(df_pfam_1)
log_data <- log(df_pfam)
sqrt_data <- sqrt(df_pfam)
par(mfrow=c(1,4))
...
```{r}
#clr on data
clr_data <- as.data.frame(clr(df_pfam))
typeof(clr_data)
tail(clr_data)
