############################################################
#                                                          #
# adapted from https://github.com/FrederickHuangLin/ANCOM  #
#  Anna Heintz-Buschart, Jan 2021                          #
#                                                          #
############################################################
...
outlier_check <- function(x,out_cut=out_cut){
...
feature_table_pre_process <- function(feature_table, meta_data, sample_var, group_var = NULL, 
  ...
  if (!is.null(group_var)) {
    ...
    out_ind[, !is.na(group)] <- t(apply(y, 1, function(i) 
      unlist(tapply(i, group, function(j) outlier_check(j,out_cut = out_cut)))))
...
ANCOM <- function(feature_table, meta_data, struc_zero = NULL, main_var, p_adj_method = "BH", 
  ...
  if (is.null(rand_formula) & is.null(adj_formula)) {
    ...
    if (length(unique(meta_data[,which(colnames(meta_data)==main_var)])) == 2) {
  ...
  out_comp$`detected_0.9` <- ifelse(W > 0.9 * (n_taxa -1), TRUE, FALSE)
  out_comp$`detected_0.8` <- ifelse(W > 0.8 * (n_taxa -1), TRUE, FALSE)
  out_comp$`detected_0.7` <- ifelse(W > 0.7 * (n_taxa -1), TRUE, FALSE)
  out_comp$`detected_0.6` <- ifelse(W > 0.6 * (n_taxa -1), TRUE, FALSE)
  ...
  if (is.matrix(eff_size)){
    ...
    fig <- ggplot(data = dat_fig) + aes(x = x, y = y) + 
      ...
      geom_text(aes(label=ifelse(out$detected_0.9 & !is.infinite(out$W),
                                 gsub(".+__","",as.character(out$taxa_id)),'')),
                hjust=0,vjust=0,size=1.5) +
      theme_bw() +
