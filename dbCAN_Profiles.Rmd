...
autoplot(few0_funct_pca_log, data=t(df_few0_funct_log), colour = rainbow(6)[as.numeric(as.factor(df_info$Treatment))])
...
#library size
#logistic
prepro <- feature_table_pre_process(feature_table = df_counts[rowSums(df_few0_funct[,df_info$Treatment %in% c("GUS","712")])>0,df_info$Treatment %in% c("GUS","MMA")], 
                                   meta_data = df_info[df_info$Treatment %in% c("GUS","MMA"),], 
                                   sample_var = "Sample", 
                                   group_var = "Treatment", 
                                   out_cut = 0.05, 
                                   zero_cut = 0.90, 
