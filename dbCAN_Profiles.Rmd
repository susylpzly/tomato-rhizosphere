...
bc1 <- vegdist(t(df_few0_funct_sum))
...
fitfil_data_clr = Maaslin2(
input_data = t(df_few0_712_GUS),
input_metadata = sInfo,
min_prevalence = 0,
transform = "NONE",
normalization = "CLR",
analysis_method = "LM",
output = "testfiltered_output_clr",
fixed_effects = c("group"),
reference = c("group,T_GUS"))

fit_data_clr = Maaslin2(
input_data = t(ko_T712),
input_metadata = sInfo_T712,
min_prevalence = 0,
transform = "NONE",
normalization = "CLR",
analysis_method = "LM",
output = "test_output_clr",
fixed_effects = c("group"),
reference = c("T_GUS"))

fit_data_css = Maaslin2(
  input_data = t(ko_T712), 
  input_metadata = sInfo_T712, 
  min_prevalence = 0,
  transform = "LOG",
  normalization = "CSS",
  analysis_method = "LM",
  output = "test_output_CSS", 
  fixed_effects = c("group"),
  reference = c("T_GUS"))

library(statmod)
fit_data_tmm = Maaslin2(
  input_data = t(ko_T712), 
  input_metadata = sInfo_T712, 
  min_prevalence = 0,
  transform = "NONE",
  ...
  output = "test_output_TMM", 
  fixed_effects = c("group"),
  reference = c("T_GUS"))

fit_data_tss = Maaslin2(
  input_data = t(ko_T712), 
  input_metadata = sInfo_T712, 
  min_prevalence = 0,
  transform = "NONE",
  normalization = "TSS",
  analysis_method = "CPLM",
  output = "test_output_TSS", 
  fixed_effects = c("group"),
  reference = c("T_GUS"))


#rownames(fit_data_clr[['results']])
#list if sig. results from maaslin 
MSLres_712_GUS <- fit_data_clr$results$feature[which(fit_data_clr$results$pval<0.05)]

MSL_712_GUS <- fitfil_data_clr$results$feature[which(fitfil_data_clr$results$pval<0.05)]
fitfil_data_clr$results$feature[which(MSL_712_GUS %in% dds1_712_GUS_res)]
```
